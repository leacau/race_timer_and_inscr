...
const participantSchema = z.object({
  ...
  dni: z.string().min(1, "DNI/ID is required"),
  birthDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, "Enter date in YYYY-MM-DD format"),
...
export function ParticipantsTable({ participants, categories }: { participants: Participant[]; categories: Category[] }) {
  ...
  const onSubmit = async (values: ParticipantFormValues) => {
    try {
      if (editingParticipant) {
        ...
        toast({ title: "Participant Updated", description: "The participant has been successfully updated." });
      } else {
        await addParticipant(values);
        toast({ title: "Participant Added", description: "The new participant has been successfully added." });
      }
      setOpen(false);
    } catch (error) {
      toast({ variant: "destructive", title: "Error", description: "Failed to save participant." });
    }
  };
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      toast({ title: "Participant Deleted", description: "The participant has been successfully deleted." });
    } catch (error) {
      toast({ variant: "destructive", title: "Error", description: "Failed to delete participant." });
    }
  };
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toast({
        ...
        description: `${result.count} participants imported.`,
      });
    } catch (error) {
      toast({
        variant: "destructive",
        title: "Import Failed",
        description: "Could not import participants from the file.",
      });
    }
  };
  ...
  const toggleTimer = (participant: Participant) => {
    if (timers[participant.id]?.isRunning) {
      ...
      toast({
        title: "Finished!",
        description: `${participant.name} ${participant.surname} finished the race.`,
      });
    } else {
      // Start timer
          ...
          <div className="flex justify-between items-start">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                {age} years old | {p.distance} | {p.gender}
              </p>
            ...
            {isAdmin && (
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                  <DropdownMenuItem onClick={() => handleOpenDialog(p)}><Edit className="mr-2 h-4 w-4" /> Edit</DropdownMenuItem>
                  <DropdownMenuItem className="text-destructive" onClick={() => handleDelete(p.id)}><Trash2 className="mr-2 h-4 w-4" /> Delete</DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
            )}
          </div>
    ...
    <div className="flex flex-col h-full">
      ...
      {isMobile ? (
        <div className="space-y-4">
          {participants.length > 0 ? participants.map(renderParticipantCard) : <p className="text-muted-foreground text-center py-8">No participants yet.</p>}
        </div>
      ) : (
      <Card>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>{editingParticipant ? "Update participant details." : "Add a new participant to the race."}</DialogDescription>
          </DialogHeader>
          <Form {...form}>
